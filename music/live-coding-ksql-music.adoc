...
Or from the KSQL Streams view, select "Create Stream" and fill out the fields as shown below.  Because of Control Center integration with Confluent Schema Registry, it automatically detects the fields `song_id` and `duration` and their respective data types.
...
We recommend using the built-in KSQL functionality in Confluent Control Center because it's where you can also manage your Kafka cluster, it has Schema Registry integration, you can inspect your topics, manage connectors to connect to other data systems, monitor performance, etc. Alternatively, you can use the KSQL CLI from the provided Docker container: 
