...
if is_cp; then confluent local config wikipedia-irc -- -d ./connector_irc.config; else confluent local config wikipedia-irc -- -d ./connector_irc_oss.config; fi
sleep 10
...
if is_cp; then confluent local config replicate-topic -- -d ./connector_replicator.config; fi
if is_cp; then confluent local config elasticsearch -- -d ./connector_elasticsearch.config; else confluent local config elasticsearch -- -d ./connector_elasticsearch_oss.config; fi
