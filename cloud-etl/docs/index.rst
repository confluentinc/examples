...
-  `Confluent CLI <https://docs.confluent.io/confluent-cli/current/install.html>`__ v2.0.0 or later, logged in with the ``--save`` argument which saves your |ccloud| user login credentials or refresh token (in the case of SSO) to the local ``netrc`` file.
