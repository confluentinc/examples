...
-  Cloud source connectors: writes data to Kafka topics in |ccloud| from a cloud service, one of:
   ...
   - `Amazon Kinesis Source Connector for Confluent Cloud <https://docs.confluent.io/cloud/current/connectors/cc-kinesis-source.html#cc-kinesis-source>`__
...
-  `Confluent Cloud CLI <https://docs.confluent.io/confluent-cli/current/install.html>`__ v1.7.0 or later, logged in with the ``--save`` argument which saves your |ccloud| user login credentials or refresh token (in the case of SSO) to the local ``netrc`` file.
...
#. Using the `Confluent Cloud CLI <https://docs.confluent.io/confluent-cli/current/install.html>`__, list all the fully-managed connectors created in this cluster.
