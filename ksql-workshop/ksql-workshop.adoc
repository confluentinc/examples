...
NOTE: On Linux and OSX the name of the network is `ksqlworkshop_default` (no `-` between `ksql` and `workshop`).
...
_On Windows you may find that the above doesn't work. If so, run just the second part of the command (starting `docker run…`) and manually paste the contents of `data/customers.json doesn't have cat (for loading customer data)
...
NOTE: The timestamp of KSQL Table events should be _before_ the event to which they are being joined. Because of KStream processing semantics you may see null value results against some events produced prior to the table events.
...
To access Kibana, first look up the address using docker ps:
[source,bash]
----
$ docker ps
71c9a39c5150        docker.elastic.co/kibana/kibana:6.3.0                                                   "/usr/local/bin/ki..."   11 minutes ago      Up 11 minutes       0.0.0.0:5601->5601/tcp                           kibana
----

Then, point your browser to that address (in this case, 0.0.0.0:5601)
