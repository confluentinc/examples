...
echo -e "\nData from Kafka topics in Confluent Cloud\n"

echo -e "$KAFKA_TOPIC_NAME_IN: confluent local consume $KAFKA_TOPIC_NAME_IN -- --cloud --from-beginning --property print.key=true --max-messages 10"
...
echo -e "\n$KAFKA_TOPIC_NAME_OUT2: confluent local consume $KAFKA_TOPIC_NAME_OUT2 -- --cloud --from-beginning --property print.key=true --max-messages 10"
