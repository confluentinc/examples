...
* Note that the requirements.txt file specifies a version of the `confluent-kafka` library >= 1.4.2 which is required for the latest Serialization API demonstrated here. If you install the library manually or globally, the same version requirements  will need to be be satisfied.
