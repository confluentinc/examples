...
7. View the schema information registered in Confluent Cloud Schema Registry. In the output below, substitute values for `{{ SR_API_KEY }}`, `{{ SR_API_SECRET }}`, and `{{ SR_ENDPOINT }}`.
    ...
    ```
8. For schema evolution: you can test schema compatibility between newer schema versions and older schema versions in Confluent Cloud Schema Registry. In the [pom.xml](pom.xml) file, set the subject name (i.e., `test2-value`) and local schema to test (in this example, [DataRecordAvro2a](src/main/resources/avro/io/confluent/examples/clients/cloud/DataRecordAvro2a.avsc) should fail and [DataRecordAvro2b](src/main/resources/avro/io/confluent/examples/clients/cloud/DataRecordAvro2b.avsc) should pass).
