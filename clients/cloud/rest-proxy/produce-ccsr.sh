...
docker-compose exec rest-proxy curl -X POST -H "Content-Type: application/vnd.schemaregistry.v1+json" --data '{ "schema": "[ { \"type\":\"record\", \"name\":\"user\", \"fields\": [ {\"name\":\"userid\",\"type\":\"long\"}]} ]" }' -u "$SCHEMA_REGISTRY_BASIC_AUTH_USER_INFO" "$SCHEMA_REGISTRY_URL/subjects/test2-value/versions"
...
docker-compose exec rest-proxy curl -X POST \
     ...
     --data '{"value_schema_id": '"$schemaid"', "records": [{"value": {"user":{"userid": 1}}}]}' \
