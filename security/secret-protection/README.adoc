...
The secrets file will contain encrypted secrets for the master encryption key, secret encryption key, and configuration parameters, along with their metadata such as which cipher was used for encryption.
...
You may also have a requirement to rotate the master encryption key or secret encryption key on a regular basis.
You can do either of these with the CLI, and the example below is for rotating just the secret encryption key.
