...
-  For simplicity in this demo which is configured as a dev setup to help with learning RBAC, ``bender`` uses the token services for client authentication (e.g. bender.properties sets ``sasl.mechanism=OAUTHBEARER``). But in production, use Kerberos or mTLS for client authentication, do not use token services.
