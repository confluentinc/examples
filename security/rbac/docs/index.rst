...
-  In this example, the ``bender`` client uses the token services for simplicity in this dev environment (e.g. bender.properties uses ``sasl.mechanism=OAUTHBEARER``). But in production, the client should use Kerberos, mTLS, or SASL/PLAIN for authentication
