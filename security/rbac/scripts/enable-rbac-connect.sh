...
echo -e "\n# Grant principal User:$USER_ADMIN_CONNECT the ResourceOwner role to Topic:connect-configs"
echo "confluent iam rbac role-binding create --principal User:$USER_ADMIN_CONNECT --role ResourceOwner --resource Topic:connect-configs --kafka-cluster $KAFKA_CLUSTER_ID"
...
echo -e "\n# Grant principal User:$USER_ADMIN_CONNECT the ResourceOwner role to Topic:connect-offsets"
echo "confluent iam rbac role-binding create --principal User:$USER_ADMIN_CONNECT --role ResourceOwner --resource Topic:connect-offsets --kafka-cluster $KAFKA_CLUSTER_ID"
...
echo -e "\n# Grant principal User:$USER_ADMIN_CONNECT the ResourceOwner role to Topic:connect-statuses"
echo "confluent iam rbac role-binding create --principal User:$USER_ADMIN_CONNECT --role ResourceOwner --resource Topic:connect-statuses --kafka-cluster $KAFKA_CLUSTER_ID"
...
echo "confluent iam rbac role-binding create --principal User:$USER_ADMIN_CONNECT --role ResourceOwner --resource Group:connect-cluster --kafka-cluster $KAFKA_CLUSTER_ID"
...
echo -e "\n# Grant principal User:$USER_ADMIN_CONNECT the ResourceOwner role to Topic:_confluent-secrets (for Secret Registry)"
echo "confluent iam rbac role-binding create --principal User:$USER_ADMIN_CONNECT --role ResourceOwner --resource Topic:_confluent-secrets --kafka-cluster $KAFKA_CLUSTER_ID"
...
echo -e "\n# Grant principal User:$USER_ADMIN_CONNECT the ResourceOwner role to Group:secret-registry (for Secret Registry)"
echo "confluent iam rbac role-binding create --principal User:$USER_ADMIN_CONNECT --role ResourceOwner --resource Group:secret-registry --kafka-cluster $KAFKA_CLUSTER_ID"
...
echo "confluent iam rbac role-binding list --principal User:$USER_ADMIN_CONNECT --kafka-cluster $KAFKA_CLUSTER_ID"
confluent iam rbac role-binding list --principal User:$USER_ADMIN_CONNECT --kafka-cluster $KAFKA_CLUSTER_ID
...
echo "confluent iam rbac role-binding create --principal User:$USER_ADMIN_CONNECT --role SecurityAdmin --kafka-cluster $KAFKA_CLUSTER_ID --connect-cluster $CONNECT_CLUSTER_ID"
...
echo "confluent iam rbac role-binding list --principal User:$USER_ADMIN_CONNECT --kafka-cluster $KAFKA_CLUSTER_ID --connect-cluster $CONNECT_CLUSTER_ID"
confluent iam rbac role-binding list --principal User:$USER_ADMIN_CONNECT --kafka-cluster $KAFKA_CLUSTER_ID --connect-cluster $CONNECT_CLUSTER_ID
...
echo -e "\n# Grant principal User:$USER_CONNECTOR_SUBMITTER the ResourceOwner role to Connector:$CONNECTOR_NAME"
echo "confluent iam rbac role-binding create --principal User:$USER_CONNECTOR_SUBMITTER --role ResourceOwner --resource Connector:$CONNECTOR_NAME --kafka-cluster $KAFKA_CLUSTER_ID --connect-cluster $CONNECT_CLUSTER_ID"
...
echo "confluent iam rbac role-binding list --principal User:$USER_CONNECTOR_SUBMITTER --kafka-cluster $KAFKA_CLUSTER_ID --connect-cluster $CONNECT_CLUSTER_ID"
confluent iam rbac role-binding list --principal User:$USER_CONNECTOR_SUBMITTER --kafka-cluster $KAFKA_CLUSTER_ID --connect-cluster $CONNECT_CLUSTER_ID

echo -e "\n# Grant principal User:$USER_CONNECTOR the ResourceOwner role to Topic:$TOPIC2_AVRO"
echo "confluent iam rbac role-binding create --principal User:$USER_CONNECTOR --role ResourceOwner --resource Topic:$TOPIC2_AVRO --kafka-cluster $KAFKA_CLUSTER_ID"
...
echo "confluent iam rbac role-binding list --principal User:$USER_CONNECTOR --kafka-cluster $KAFKA_CLUSTER_ID"
confluent iam rbac role-binding list --principal User:$USER_CONNECTOR --kafka-cluster $KAFKA_CLUSTER_ID

get_cluster_id_schema_registry
echo -e "\n# Grant principal User:$USER_CONNECTOR the ResourceOwner role to Subject:${TOPIC2_AVRO}-value"
echo "confluent iam rbac role-binding create --principal User:$USER_CONNECTOR --role ResourceOwner --resource Subject:${TOPIC2_AVRO}-value --kafka-cluster $KAFKA_CLUSTER_ID --schema-registry-cluster $SCHEMA_REGISTRY_CLUSTER_ID"
...
echo "confluent iam rbac role-binding list --principal User:$USER_CONNECTOR --kafka-cluster $KAFKA_CLUSTER_ID --schema-registry-cluster $SCHEMA_REGISTRY_CLUSTER_ID"
confluent iam rbac role-binding list --principal User:$USER_CONNECTOR --kafka-cluster $KAFKA_CLUSTER_ID --schema-registry-cluster $SCHEMA_REGISTRY_CLUSTER_ID
...
# Consume messages from the $TOPIC2_AVRO topic
echo -e "#\n Grant principal User:$USER_CONNECTOR the DeveloperRead role to Group:console-consumer- prefix"
echo "confluent iam rbac role-binding create --principal User:$USER_CONNECTOR --role DeveloperRead --resource Group:console-consumer- --prefix --kafka-cluster $KAFKA_CLUSTER_ID"
confluent iam rbac role-binding create --principal User:$USER_CONNECTOR --role DeveloperRead --resource Group:console-consumer- --prefix --kafka-cluster $KAFKA_CLUSTER_ID
