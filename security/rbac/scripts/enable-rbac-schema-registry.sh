...
echo -e "\n# Grant principal User:$USER_ADMIN_SCHEMA_REGISTRY the ResourceOwner role to Topic:_schemas"
echo "confluent iam rbac role-binding create --principal User:$USER_ADMIN_SCHEMA_REGISTRY --role ResourceOwner --resource Topic:_schemas --kafka-cluster $KAFKA_CLUSTER_ID"
...
confluent iam rbac role-binding create --principal User:$USER_ADMIN_SCHEMA_REGISTRY --role ResourceOwner --resource Group:schema-registry-demo --kafka-cluster $KAFKA_CLUSTER_ID

echo -e "\n# Grant principal User:$USER_ADMIN_SCHEMA_REGISTRY the DeveloperRead and DeveloperWrite roles to Topic:$LICENSE_TOPIC"
echo "confluent iam rbac role-binding create --principal User:$USER_ADMIN_SCHEMA_REGISTRY --role DeveloperRead --resource Topic:$LICENSE_TOPIC --kafka-cluster $KAFKA_CLUSTER_ID"
echo "confluent iam rbac role-binding create --principal User:$USER_ADMIN_SCHEMA_REGISTRY --role DeveloperWrite --resource Topic:$LICENSE_TOPIC --kafka-cluster $KAFKA_CLUSTER_ID"
...
echo "confluent iam rbac role-binding create --principal User:$USER_ADMIN_SCHEMA_REGISTRY --role SecurityAdmin --kafka-cluster $KAFKA_CLUSTER_ID --schema-registry-cluster $SCHEMA_REGISTRY_CLUSTER_ID"
...
echo "confluent iam rbac role-binding list --principal User:$USER_ADMIN_SCHEMA_REGISTRY --kafka-cluster $KAFKA_CLUSTER_ID"
confluent iam rbac role-binding list --principal User:$USER_ADMIN_SCHEMA_REGISTRY --kafka-cluster $KAFKA_CLUSTER_ID

echo -e "\n# List the role bindings for User:$USER_ADMIN_SCHEMA_REGISTRY to the Schema Registry cluster"
echo "confluent iam rbac role-binding list --principal User:$USER_ADMIN_SCHEMA_REGISTRY --kafka-cluster $KAFKA_CLUSTER_ID --schema-registry-cluster $SCHEMA_REGISTRY_CLUSTER_ID"
confluent iam rbac role-binding list --principal User:$USER_ADMIN_SCHEMA_REGISTRY --kafka-cluster $KAFKA_CLUSTER_ID --schema-registry-cluster $SCHEMA_REGISTRY_CLUSTER_ID
...
echo -e "\n# Grant principal User:$USER_CLIENT_A the ResourceOwner role to Subject:$SUBJECT"
echo "confluent iam rbac role-binding create --principal User:$USER_CLIENT_A --role ResourceOwner --resource Subject:$SUBJECT --kafka-cluster $KAFKA_CLUSTER_ID --schema-registry-cluster $SCHEMA_REGISTRY_CLUSTER_ID"
...
echo -e "\n# List the role bindings for User:$USER_CLIENT_A"
echo "confluent iam rbac role-binding list --principal User:$USER_CLIENT_A --kafka-cluster $KAFKA_CLUSTER_ID --schema-registry-cluster $SCHEMA_REGISTRY_CLUSTER_ID"
confluent iam rbac role-binding list --principal User:$USER_CLIENT_A --kafka-cluster $KAFKA_CLUSTER_ID --schema-registry-cluster $SCHEMA_REGISTRY_CLUSTER_ID
